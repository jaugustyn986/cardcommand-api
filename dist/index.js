...
async function seedDatabase() {
    try {
        ...
        const dealCount = await database_2.prisma.deal.count().catch(() => 0);
        if (dealCount === 0) {
            ...
            for (const deal of deals) {
                await database_2.prisma.deal.upsert({
                    where: { id: deal.id },
                    update: {},
                    create: deal,
                });
...
// CORS - Allow Vercel frontend and preview deployments
...
app.get('/health', (req, res) => {
    res.status(200).json({
        ...
        database: database_1.isConnected ? 'connected' : 'disconnected',
    });
});
// ============================================
// API Routes
